...
export const adminProductService = {
  ...
  updateStock: async (id: number, quantity: number): Promise<Product> => {
    const response = await adminApiClient.put<AdminApiResponse<Product>>(
      `/admin/inventory/products/${id}/stock`,
      { operation: 'set', quantity, reason: '在庫手動更新' }
    );
    return response.data.data;
  },

  // 商品の一括在庫更新
  bulkUpdateStock: async (updates: Array<{ product_id: number; quantity: number }>): Promise<void> => {
    await adminApiClient.put('/admin/products/bulk/stock', { updates });
  },
};
