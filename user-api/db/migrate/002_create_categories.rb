class CreateCategories < ActiveRecord::Migration[7.0]
  def change
    ...
    add_index :categories, [:parent_id, :sort_order] unless index_exists?(:categories, [:parent_id, :sort_order])
    add_index :categories, [:slug, :is_active] unless index_exists?(:categories, [:slug, :is_active])
    add_foreign_key :categories, :categories, column: :parent_id, on_delete: :cascade, validate: false unless foreign_key_exists?(:categories, :categories)
  end
end
